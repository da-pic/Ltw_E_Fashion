...
public class CartDAO {
    ...
    public Cart getCartByUserId(String userId) {
        ...
        String sql = "SELECT ci.cart_id, ci.product_variant_id, ci.amount, ci.unit_price, "
                   + "pv.color, pv.size, pv.image, pv.import_price, pv.stock "
                   + "FROM cart_items ci "
                   + "JOIN product_variants pv ON ci.product_variant_id = pv.id "
                   + "WHERE ci.cart_id = ?";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                ProductVariant productVariant = new ProductVariantDAO().getProductVariantsById(
                        rs.getString("product_variant_id"));
                ProductVariant variant = new ProductVariant(
                    rs.getString("product_variant_id"), productVariant.getProduct_id(), rs.getString("color"), 
                    rs.getString("size"), rs.getString("image"), 
                    productVariant.getImportPrice(), rs.getInt("unit_price"), 
