...
public class ProductDAO {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                Product product = new Product(
                    rs.getString("id"), rs.getString("product_name"), rs.getString("brand_id"), null,
                    rs.getInt("category_id"), rs.getBoolean("is_active"), rs.getString("description"),
                    rs.getInt("display_price"), rs.getString("display_image")
                );
    ...
    public List<Product> getFlashSaleProducts() {
        ...
        String sql = "SELECT p.*, "
                   ...
                   + " WHERE c.is_active = 1 "
                   + " AND (c.type = 'PRODUCT' OR c.type = 'product' OR c.type = '%') " 
